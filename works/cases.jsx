  ...
  return (
    <section id="w-case" className="w-section" style={{background: 'var(--w-ink)'}}>
      <style>{`
        ...
        .w-case-filter { padding: 10px 16px; background: transparent; border: 1px solid var(--w-line); color: var(--w-text-soft); font-family: "Inter", sans-serif; font-size: 11px; font-weight: 500; letter-spacing: 0.18em; text-transform: uppercase; cursor: pointer; transition: all .25s; }
        ...
        .w-case { position: relative; background: var(--w-ink-2); border: 1px solid var(--w-line); overflow: hidden; aspect-ratio: 4/5; cursor: pointer; transition: transform .4s cubic-bezier(.2,.7,.2,1), border-color .25s; }
        ...
        .w-case-img { position: absolute; inset: 0; background: repeating-linear-gradient(135deg, rgba(244,241,234,0.05) 0 8px, rgba(244,241,234,0.01) 8px 16px), var(--w-ink-3); transition: transform .6s cubic-bezier(.2,.7,.2,1); display: flex; align-items: center; justify-content: center; }
        .w-case-img-tag { font-family: "JetBrains Mono", monospace; font-size: 10px; letter-spacing: 0.18em; color: var(--w-text-mute); padding: 4px 10px; border: 1px solid var(--w-line); text-transform: uppercase; }
        .w-case-overlay { position: absolute; inset: 0; background: linear-gradient(180deg, transparent 30%, rgba(10,22,40,0.92) 100%); opacity: 0.85; transition: opacity .3s; }
        ...
        .w-case-num { font-family: "JetBrains Mono", monospace; font-size: 10px; letter-spacing: 0.16em; color: var(--w-accent); background: rgba(10,22,40,0.7); padding: 4px 8px; backdrop-filter: blur(8px); }
        .w-case-cat { font-family: "Inter", sans-serif; font-size: 9px; font-weight: 600; letter-spacing: 0.22em; padding: 4px 10px; background: var(--w-accent); color: #0A1628; }
        ...
        .w-case-en { font-family: "Inter", sans-serif; font-size: 10px; letter-spacing: 0.2em; color: var(--w-text-mute); text-transform: uppercase; }
      `}</style>

      <div className="w-section-head">
        <div>
          <div className="w-eyebrow" style={{marginBottom: 24}}>CASE · 歷年案例</div>
          <h2><em>作品集</em>，<br/>不說也行。</h2>
        </div>
      ...
      <div className="w-case-tools">
        <div className="w-case-filters">
          {cats.map(c => (
            <button key={c} className={'w-case-filter' + (filter === c ? ' active' : '')} onClick={() => setFilter(c)}>{c}</button>
          ))}
        </div>
      ...
      <div className="w-case-grid">
        {list.map(c => (
          <article key={c.num} className="w-case">
            <div className="w-case-img"><span className="w-case-img-tag">{c.cat} · {c.year}</span></div>
